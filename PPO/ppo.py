...
class ActorCritic(keras.Model):
    def __init__(self, state_space, action_space,  bound):
        ...
        # action
        ac_input = keras.layers.Input(shape=self.state_space)
        ac_layer1 = keras.layers.Dense(64, activation='tanh')(ac_input)
        ac_layer2 = keras.layers.Dense(64, activation='tanh')(ac_layer1)
        ...
        # critic
        cr_input = keras.layers.Input(shape=self.state_space)
        cr_layer1 = keras.layers.Dense(64, activation='tanh')(cr_input)
        cr_layer2 = keras.layers.Dense(64, activation='tanh')(cr_layer1)
    ...
    for interation in range(interations):
        ...
        for t in range(n_fixed_steps):
            ...
            for step in range(n_max_steps):
                ...
                rewards += reward
                # tf.summary.scalar('reward_{}'.format(interation),float(rewards), step=global_steps)
            ...
            # print("t = {}, global_steps = {}".format(t, global_steps))
            ...
            with tf.GradientTape(persistent=True) as tape:
                ...
                a_loss = tf.math.minimum(rio * all_advantages, cliped_rio * all_advantages)
                ac_loss = -tf.reduce_mean(a_loss)
                
                c_loss = keras.losses.mean_squared_error(cr_values, all_rewards)
                cr_loss = tf.reduce_mean(c_loss)

                print("interation = {} epoch: {} , ac_loss = {}, cr_loss = {}".format(interation, t, ac_loss, cr_loss))

            # Update Gradient
            new_net.critic.trainable = False
            new_net.actor.trainable=True
            ...
            with writer.as_default():
                tf.summary.scalar('policy_loss_{}'.format(t), ac_loss, t)
                tf.summary.scalar('value_loss_{}'.format(t), cr_loss, t)

            if global_steps > n_fixed_steps:
                break
